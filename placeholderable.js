...
// 画像生成関数
function placeholderableGeneratePosterImage(element) {
    // URLからパラメータを解析する関数
    function getParams(url) {
        const queryString = url.split('?')[1];
        if (!queryString) {
    ...
    // パラメータの取得
    const params = getParams(element.poster);
    const width = parseInt(params.width || defaultWidth);
    const height = parseInt(params.height || defaultHeight);
    const text = decodeURIComponent(params.text || `${width}x${height}`);
    const icon = decodeURIComponent(params.icon || `${defaultBootstrapIcon}`);
    ...
    // imgタグのsrcにcanvasの内容を設定
    element.poster = canvas.toDataURL('image/png');
}
...
function placeholderable() {
    ...
    const elements_poster = document.querySelectorAll('video[poster*=" "]');
    elements_poster.forEach(placeholderableGeneratePosterImage);
}
